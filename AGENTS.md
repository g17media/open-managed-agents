...
| **web_search** | Web search | Backend set by `WEB_SEARCH_PROVIDER` (`ddg` default, `tavily`, `brave`, `exa`, `serper`, `native`); keyed providers need their `*_API_KEY`. `native` drops the function tool and the pi runtime splices the provider's server tool into the request (`apps/agent/src/harness/web-search.ts`); it is gated by `nativeWebSearchRequested()` (provider native + web_search enabled + `always_allow`) and only on the vendor's own endpoint. Limitation: Pi does not replay server-tool result blocks, so search context is lost on later turns and Anthropic `pause_turn` is not resumed — prefer a keyed provider where multi-turn research matters. Agent `allowed_domains` / `blocked_domains` / `user_location` on the `web_search` config entry are forwarded or post-filtered on every backend. |
