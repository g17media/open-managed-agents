...
| `WEB_SEARCH_PROVIDER` | No | Backend for the `web_search` built-in tool: `ddg` (default, keyless DuckDuckGo scrape — rate limited from shared egress IPs), `tavily`, `brave`, `exa`, `serper`, or `native` (the model provider's own search tool). Unset picks the first keyed provider whose key is present. `native` applies only to Anthropic `claude-*` and OpenAI model cards that point at the vendor's own endpoint (no gateways), with `web_search` on `always_allow` and at most one of `allowed_domains` / `blocked_domains` for Anthropic; everything else falls back to a keyed provider. Known limitation of `native`: the Pi runtime replays only text, thinking and function-call blocks, so the provider's search result blocks are not echoed on later turns — the model answers from the search in the turn it ran but loses that search context afterwards, and an Anthropic `pause_turn` cannot be resumed. Keyed providers have no such limit. |
