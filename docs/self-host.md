...
| `BelljarSandbox` | Container isolation on your own Docker/Podman host — the same `cloudflare/sandbox` runtime as the CF path, no CF account | `SANDBOX_PROVIDER=belljar`, `BELLJAR_URL=http://belljar:8877`, `BELLJAR_TOKEN=...` (required beyond loopback). Run [belljar](https://github.com/g17media/belljar) next to a Docker/Podman socket. `SANDBOX_IMAGE` must be a `cloudflare/sandbox`(-derived) image — leave unset to use the belljar server's `BELLJAR_IMAGE` default. An environment's `config.image` overrides `SANDBOX_IMAGE` per session (same `cloudflare/sandbox`-derived requirement), and `config.image_registry_auth` may name a vault credential (auth type `container_registry`) for pulling it from a private registry — resolved control-plane-side and handed to belljar for the pull only; the credential never enters the sandbox and is never persisted in belljar's labels or tombstones. Idle sandboxes sleep after `BELLJAR_SLEEP_AFTER` and wake on the next request; `/workspace` survives sleep. Memory/outputs mounts are Docker binds fixed at creation: allowlist the data dir's host path via `BELLJAR_ALLOWED_MOUNT_ROOTS` on the belljar server, and set `BELLJAR_MOUNT_PATH_MAP=/app/data=<host data dir>` on OMA when it runs in a container. `BELLJAR_ISOLATION=1` (compose default) network-isolates each sandbox on an internal no-gateway Docker network; when `OMA_VAULT_PROXY_URL` names the oma-vault container, belljar attaches it to that network as the sandbox's **only** egress — the vault proxy becomes kernel-enforced instead of advisory env. Vault URL must be a container name (not localhost/host.docker.internal) for this to engage. |
...
| `Belljar` | ✓ | ✓ | ✓ | ✓ (engine bind mount, fixed at creation; needs `BELLJAR_ALLOWED_MOUNT_ROOTS` on the belljar server + `BELLJAR_MOUNT_PATH_MAP` when OMA is containerized; read-only enforced by `:ro` bind) | ✓ (same bind mechanism) | ✓ (CA written into the container via the runtime write API; proxy URL must be routable from the sandbox container — not localhost) | ✓ (tar via exec + readFileBytes) |
...
- `/v1/oma/skills` upload and versions, `/v1/oma/clawhub` search/install
- `/v1/files` upload/list/get/content/delete
- `/v1/oma/oauth` (MCP OAuth), `/v1/oma/cap-cli/oauth` (device flow)
- MCP tools use the shared credential-injection proxy on Node and Cloudflare.
- In-memory rate limiting on `/auth/*` and `/v1/*` writes.
