        ...
        <VaultSwapModal
          ...
          onSwapped={(next) =>
            setSessionMeta((prev) => ({
              ...
              vaults: next.map((v) => ({ id: v.id, name: v.name })),
            }))
          }
        />
      )}
...
function VaultSwapModal({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    managedApi.vaults.list({ limit: 200 })
      .then((res) => setVaults(res.data.map((v) => ({ id: v.id, name: v.display_name }))))
      .catch(() => {})
  ...
  const save = async () => {
    setSaving(true);
    try {
      await managedApi.sessions.update(sessionId, { vault_ids: selected });
