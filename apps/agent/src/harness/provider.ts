...
export function modelCallOptions(
  ...
  // `provider_options` is the raw escape hatch: merged LAST so an explicit
  // setting always beats what speed/reasoning/effort derived above. Merged
  // per provider key rather than wholesale, so supplying e.g. `openai` does
  // not discard a derived `anthropic` block.
  const raw = spec.provider_options as
