...
import { checkUploadFreq } from "../quotas";
...
// Reuses the zip limits + persistence from skills.ts; the only genuinely new
// logic is multi-skill discovery and the name-based upsert.
//
// Kept in its own module (mounted on the same /v1/skills base, before
// skills.ts) so the upstream skills.ts diff stays minimal.
// ---------------------------------------------------------------------------
 ...
 *  skills.ts still apply to the uncompressed contents. */
...
> {
  ...
  const keys = await kvListAll(services.kv, kvPrefix(tenantId, "skill"));
  ...
  for (const d of discovered) {
    ...
    if (match && match.source === "custom") {
      ...
      const r = await persistNewVersion(env, bucket, tenantId, match.id, {
        files: d.files,
      ...
      const r = await persistNewSkill(env, bucket, tenantId, {
        files: d.files,
        name: d.name,
        description: d.description,
        github_source: provenance,
...
app.post("/import/github", async (c) => {
  ...
  const result = await importGitHubSource(c.env, c.var.services, bucket, t, {
    repo: parsed.repo,
    ref,
    path,
    token: body.token,
...
app.post("/sync/github", async (c) => {
  ...
  const keys = await kvListAll(c.var.services.kv, kvPrefix(t, "skill"));
