...
const sandboxConfiguration = createSandboxConfiguration({
  sql, sessionsService, credentialService, environmentsService, logger,
  nativeCredentials: () => managedCredentialStore,
});
...
async function buildSandbox(
  ...
  const configuration = await sandboxConfiguration(sessionId, selection.provider, managed);
...
const sessionPreparation = createNodeSessionPreparation({
  sql, sessionsService, memoryService, getKv: () => kv, filesBlob, filesService,
  environmentsService, sessionOutputs, logger,
});
...
const sessionRegistry = new SessionRegistry({
  ...
  buildHarnessContext: async (input) => {
    ...
    const memoryReminders = await sessionPreparation.reminders(input);
    return {
      agent: input.agent,
...
const managedPreparation = createManagedSessionPreparation({
  memoryRoot: memoryBlobLocalDir ?? "./data/memory",
  memoriesForSession: (workspaceId, sessionId) => managedMemoriesApplicationFor({ var: { tenant_id: workspaceId, session_id: sessionId } }).port(managedAgentsPortTokens.memories),
  filesFor: (workspaceId) => managedAgentsPlatform.app({ workspaceId }).port(managedAgentsPortTokens.files),
  sessionOutputs, sandboxOrchestrator, logger,
});
...
const managedRuntimeRunner = new DefaultNodeManagedSessionRunner({
  ...
  buildSandbox: async ({ workspaceId, session, environment }) => {
    // OpenAI-agents sessions without an environment get a stub sandbox —
    // there is no workspace to scope, mount or provision.
    if (await isManagedNoEnvironmentSession(session, openAIAgentsSecrets)) {
      return createNoEnvironmentSandbox();
    }
...
function managedMemoriesApplicationForWorkspace(
  workspaceId: string,
  userId?: string,
  /** Attribute memory versions to the session when the write comes from one;
   *  agent-authored edits belong to the session, not the signed-in user. */
  sessionId?: string,
) {
  return createNodeManagedAgentsApp({
    ...
    modules: () => [
      ...
      providePort(
        memoryVersionActorPort,
        sessionId ? { kind: "session", sessionId } : managedMemoryActor(userId),
      ),
    ],
  });
}
...
app.route("/internal/belljar/lifecycle", buildBelljarLifecycleRoutes({
  ...
  prepare: async (session, sandbox, trigger) => {
    ...
    else await sessionPreparation.mountSessionResources({ sessionId: session.id, tenantId: session.tenant_id, sandbox, strict: true });
  },
}));
...
v1.route("/oma/sessions", buildSessionRoutes({
  ...
  outputs: nodeOutputsAdapter(outputsRoot),
  lifecycle: {
    // Upstream hoisted the hooks into `nodeSessionLifecycleHooks` (and added
    // `outputs`); spread it rather than rebuilding, so future upstream fields
    // come along for free. The rate-limit gate is ours.
