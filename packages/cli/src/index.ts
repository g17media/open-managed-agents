...
async function connectMcp(config: Config, mcpServerUrl: string, vaultId: string) {
  ...
  const tenantParam = config.tenantId
    ...
    : "";
  const authUrl = `${config.baseUrl}/v1/oma/oauth/authorize?mcp_server_url=${encodeURIComponent(mcpServerUrl)}&vault_id=${encodeURIComponent(vaultId)}&redirect_uri=${encodeURIComponent(redirectUri)}${tenantParam}`;
