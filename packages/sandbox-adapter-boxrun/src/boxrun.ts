...
import type { SandboxExecutor, SandboxFactory } from "@open-managed-agents/sandbox";
...
export class BoxRunSandbox implements SandboxExecutor {
  ...
  async writeFileBytes(path: string, bytes: Uint8Array): Promise<string> {
    ...
    const { dir, name } = tarUploadTarget(path);
...
function tarUploadTarget(path: string): { dir: string; name: string } {
...
function extractFirstRegularFile(tar: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < 20 && off + 512 <= tar.length; i++) {
    ...
    if (typeflag === 0 || typeflag === 48) {
      return tar.subarray(off, off + contentSize);
    }
    off += Math.ceil(contentSize / 512) * 512;
  }
