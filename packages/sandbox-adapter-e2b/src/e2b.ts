...
import { readS3MemoryBucket } from "@open-managed-agents/sandbox";
...
export class E2BSandboxExecutor
  ...
  constructor(
    ...
    this.logger = opts.logger ?? {
      warn: (msg) => console.warn(`[e2b-sandbox] ${msg}`),
    };
  ...
  async exec(command: string, timeout?: number): Promise<string> {
    ...
    const combined =
      (result.stdout + (result.stderr ? `${separator}${result.stderr}` : "")).replace(/\s+$/, "") +
  ...
  async setOutboundContext(opts?: { tenantId: string; sessionId: string }): Promise<void> {
    ...
    // This executor is created only after the E2B sandbox exists, so the CA
    // upload is immediate. Provider creation remains the only lazy boundary.
    try {
      const { promises: nodeFs } = await import("node:fs");
      const buf = await nodeFs.readFile(caCertPath);
      await this.sandbox.files.write(inBoxCaPath, buf);
    } catch (err) {
