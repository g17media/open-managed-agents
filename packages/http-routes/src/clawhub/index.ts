...
import { kvKey } from "../lib/kv-helpers";

export interface ClawhubRoutesDeps {
  services: RouteServicesArg;
}

export function buildClawhubRoutes(deps: ClawhubRoutesDeps) {
  ...
  app.post("/install", async (c) => {
    ...
    await Promise.all([
      services.kv.put(kvKey(t, "skill", id), JSON.stringify(skill)),
      services.kv.put(kvKey(t, "skillver", id, versionId), JSON.stringify(version)),
    ]);
