...
export function buildCapCliOauthRoutes(deps: CapCliOauthRoutesDeps) {
  ...
  app.post("/initiate", async (c) => {
    ...
    await services.kv.put(kvSessionKey(sessionId), JSON.stringify(session), {
  ...
  app.post("/poll", async (c) => {
    ...
    const raw = await services.kv.get(kvSessionKey(body.session_id));
    ...
    switch (result.kind) {
      ...
      case "slow_down": {
        ...
        await services.kv.put(kvSessionKey(body.session_id), JSON.stringify(updated), {
          expirationTtl: DEVICE_FLOW_KV_TTL_SEC,
        });
      ...
      case "ready": {
        ...
        try {
          const existing = await services.credentials.list({
            tenantId,
          ...
          for (const ec of existing) {
            ...
            await services.credentials.archive({
              tenantId,
              vaultId: session.vault_id,
